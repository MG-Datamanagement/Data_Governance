...
async def create_table(
    ...
    try:
        ...
        # Check for duplicate table name within data source
        ...
        logger.error("Failed to create table", error=str(e), exc_info=True)
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def add_table_to_favorites(
    ...
    try:
        ...
        # In production, you would add to user's favorites table
        logger.info("Table added to favorites", table_id=table_id)
...
async def remove_table_from_favorites(
    ...
    try:
        ...
        # For demo purposes, just return success
        # In production, you would remove from user's favorites table
        logger.info("Table removed from favorites", table_id=table_id)
