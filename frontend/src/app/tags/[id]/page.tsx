...
export default function TagDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchTag = async () => {
      try {
        const response = await fetch(`https://nmqhfvs3-8001.inc1.devtunnels.ms/api/v1/tags/${params.id}`);
        if (!response.ok) {
        ...
        const tablesResponse = await fetch(`https://nmqhfvs3-8001.inc1.devtunnels.ms/api/v1/tables?page=1&per_page=100`);
