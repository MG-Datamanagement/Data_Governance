...
export default function DomainDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchDomain = async () => {
      try {
        const response = await fetch(`https://nmqhfvs3-8001.inc1.devtunnels.ms/api/v1/domains/${params.id}`);
        if (!response.ok) {
        ...
        const tablesResponse = await fetch(`https://nmqhfvs3-8001.inc1.devtunnels.ms/api/v1/tables?domain_ids=${params.id}`);
