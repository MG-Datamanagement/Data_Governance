...
const LineageGraph: React.FC<LineageGraphProps> = (props: LineageGraphProps) => {
  ...
  const processLineageData = useCallback((data: LineageData) => {
    ...
    nodes.set(centerTable.table_id, {
      ...
      dataSourceBase64Url: centerTable.data_source_base64_url,
      type: 'center',
      depth: 0
    });
    ...
    data.upstream_links.forEach(link => {
      ...
      if (!nodes.has(src.id)) {
        nodes.set(src.id, {
          ...
          dataSourceBase64Url: src.data_source_base64_url,
          type: 'upstream',
          depth: -link.depth
        });
      }
    ...
    data.downstream_links.forEach(link => {
      ...
      if (!nodes.has(tgt.id)) {
        nodes.set(tgt.id, {
          ...
          dataSourceBase64Url: tgt.data_source_base64_url,
          type: 'downstream',
          depth: link.depth
        });
      }
  ...
  useEffect(() => {
    ...
    // Node label group
    const labelGroup = nodes.append('g')
      .attr('class', 'node-label')
      .attr('transform', `translate(${nodeWidth / 2}, ${nodeHeight / 2})`);

    // Background for layout reference (optional, for debugging)
    ...
    labelGroup.append('image')
      .attr('x', -nodeWidth / 2 + 15) // padding left
      .attr('y', -12)
      .attr('width', 24)
      .attr('height', 22)
      .attr('preserveAspectRatio', 'xMidYMid meet')
      .attr('href', d => d.dataSourceBase64Url || null);

    // Text
    labelGroup.append('text')
      .attr('x', -nodeWidth / 2 + 45) // after icon
      .attr('y', 3)
      .attr('text-anchor', 'start')
      .attr('fill', '#181d27')
      .attr('font-size', '14px')
      .attr('font-weight', '600')
      .attr('width', nodeWidth - 45)
      .attr('style', 'pointer-events: none;')
      .text(d => formatNodeLabel(d.name))
      .each(function (d) {
        const text = d3.select(this);
        const maxWidth = nodeWidth - 50;
        const fullText = formatNodeLabel(d.name);
