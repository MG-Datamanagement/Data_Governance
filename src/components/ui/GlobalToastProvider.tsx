  ...
  return (
    <div className="fixed bottom-8 right-8 z-[100] flex flex-col items-end gap-3 pointer-events-none">
      {toasts.map((toast) => (
        ...
        >
          <div
            className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 ${toast.type === "success" ? "bg-green-500" : toast.type === "error" ? "bg-red-500" : toast.type === "warning" ? "bg-orange-500" : "bg-blue-500"}`}
          >
            {toast.type === "success" && (
