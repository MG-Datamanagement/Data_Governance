...
const IngestionSidebar: React.FC<IngestionSidebarProps> = ({ jobId, sourceName, isOpen, onClose }) => {
    ...
    const advanceStep = useCallback((idx: number, stepList: Step[], onComplete?: () => void) => {
        ...
        const progressVal = Math.min(idx + 1, stepList.length);
        setProgress(progressVal);

        stepTimerRef.current = setTimeout(() => {
            setCompletedSteps(prev => new Set(prev).add(idx));

            // Auto-scroll logic: keep the bottom accessible, especially useful for CTAs
            if (scrollRef.current) {
                const isNearEnd = idx > stepList.length * 0.7;
                if (isNearEnd) {
    ...
    const fetchInitialSteps = useCallback(async () => {
        ...
        try {
            ...
            if (!addDsConfig?.piiApproval) {
                ...
                handleBatchApis(); // Trigger classification in parallel
            }

            // We always calculate progress relative to the FULL sequence (allSteps)
            // But if approval is required, we only ANIMATE the first part initially
            const animationSteps = !addDsConfig?.piiApproval ? allSteps : ingestionSteps;
            
            setTimeout(() => {
                advanceStep(0, animationSteps, () => {
                    if (!addDsConfig?.piiApproval) {
                        // Entire unified sequence is done
                        setPhase("done");
                        setStreamStatus("completed");
                    } else {
                        // Only Phase 1 is done, waiting for approval
                        setStreamStatus("completed");
                        // Force scroll to show CTA
                        setTimeout(() => {
                            scrollRef.current?.scrollTo({ top: scrollRef.current.scrollHeight, behavior: 'smooth' });
                        }, 500);
                    }
    ...
    const appendPostIngestionSteps = useCallback(async () => {
        ...
        // If we already have the full set of steps (from an upfront unified fetch),
        // we don't need to fetch or append again. Just resume the animation.
    ...
    useEffect(() => {
        if (phase === "done" && !isSourceAiSummaryLoading && streamStatus === "completed") {
            const timer = setTimeout(() => {
                setIsComplete(true);
                setIsThinking(false);
            }, 500); // Tiny buffer for smoothness
    ...
    useEffect(() => {
        ...
        // Add a mock log to show it connected
        setLogs(prev => prev.length === 0 ? [{
            timestamp: new Date().toISOString(),
            level: "info",
            message: "Connected to data source stream."
        }] : prev);
    ...
    const handleNewLog = (log: IngestionLog) => {
        ...
        if (msg.includes("Ingestion job completed")) {
            // Log indicates Phase 1 completion
            if (addDsConfig?.piiApproval) {
                setIsProgressExpanded(false);
            }
        }
